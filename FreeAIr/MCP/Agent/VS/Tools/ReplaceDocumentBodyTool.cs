...
{
    public sealed class ReplaceDocumentBodyTool : VisualStudioAgentTool
    {
            ...
            )
        {
            ...
            {
                return new AgentToolCallResult($"File {itemNamePath} does not found at the disk.");
            }

            if (!arguments.TryGetValue(NewItemBodyParameterName, out var newItemBodyObj))
            {
                return new AgentToolCallResult($"Parameter {NewItemBodyParameterName} does not found.");
            }
