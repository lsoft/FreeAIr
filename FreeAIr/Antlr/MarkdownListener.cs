...
namespace FreeAIr.Antlr
{
    ...
    /// <see cref="IMentionRecognizer"/> instances, and accumulates the result in a
    /// <see cref="Parsed"/>.
    /// </summary>
    public class MarkdownListener : PromptMarkdownBaseListener
    {
        /// <summary>The mention recognizers that own each anchor symbol (e.g. @-mentions) in the walked markdown.</summary>
        ...
        /// <summary>
        /// Prepares the listener with the mention recognizers that own the anchor symbols
        /// (such as @-mentions) used in the markdown being walked.
        /// </summary>
        public MarkdownListener(
            IReadOnlyList<IMentionRecognizer> recognizers
            )
        {
        ...
        /// <summary>
        /// Classifies one leaf token: if it starts with a registered recognizer's anchor symbol it
        ...
        {
            var recognizer = _recognizers.FirstOrDefault(
                r => word.StartsWith(r.AnchorSymbol.ToString())
                );
            ...
            else
            {
                ...
                var part = recognizer.CreatePart(partPayload);
                if (part is null)
                {
